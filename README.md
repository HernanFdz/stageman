...
Every project talks on Slack through an app installed in the workspace its
people talk in, and the app hears every channel it is invited to. The app is
one the instance owns, installed on a workspace with a press, or one the
project owns, set up by hand.

**Once per instance:** on the Instance page, *New app* opens Slack's form
with this manifest filled in, the address Slack brings you back to included.
Create the app, generate an app-level token under its Basic Information with
...
**Or give a project an app of its own**, the way every project was set up
before: create it from the same manifest, install it, and give the project
the bot token and an app-level token generated by hand, both checked against
Slack before they are kept. A project made either way can be moved to the
other from its settings.
...
oauth_config:
  ...
  scopes:
    bot:
      ...
      - reactions:write      # an eyes reaction says your message was received
settings:
  event_subscriptions:
    bot_events:
      - message.channels
...
**An app a project owns is that project's alone.** Slack hands each event to
one of an app's open connections, so two projects sharing such an app would
each hear half of what is said, and nothing would say so; a second project
given the same app is refused. The instance's own app is listened to on one
connection for every workspace and project, which is why it can be shared.
Where several projects share one workspace, a mention in a channel none of
them owns is answered with the names of their foremen's channels, and
watching a channel is asked from the project's foreman channel, naming it.
...
working; then the projects. It is where you add projects, name the kits each
one offers and set the credentials each one needs, watch jobs and read their
...
it ran on, the pull requests it says it opened, and links to its channel and
to what it is showing. It follows
