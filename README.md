...
the app at api.slack.com from this manifest — the project's settings page
links to Slack's form with it filled in — install it, and give the project
the bot token and an app-level token with the `connections:write` scope,
which is generated by hand under the app's Basic Information, since the
manifest cannot mint one. Both are checked against Slack before they are
kept:
