...
- **Can Slack's setup be one redirect?** An operator would rather be sent
  to the platform, press once, and come back with everything in hand,
  which is what
  `docs/decisions/0076-a-credential-is-guided-in-and-checked-before-it-is-kept.md`
  looked for and found half of. The install redirect is the half that
  exists: it hands back the bot token and the workspace, and the
  platform's settings accepted an address on localhost with a plain http
  scheme on 2026-09-23, so it is not only for a domain-hosted instance.
  Two things stand in the way of the rest, each the platform's: an app is
  created by a person, or by the manifest API with a configuration token
  that a person generates on the platform and that lives twelve hours;
  and the app-level token that opens the event stream is generated on the
  app's page and by no API. Under an app the instance owns and installs
  per workspace both happen once per instance rather than once per
  project, which is the floor: one app created from a link, one token
  generated by hand, and every workspace and project after that a press.
  Reading events over HTTPS instead of the socket would remove the second
  and was refused, for the reason
  `docs/decisions/0029-a-reply-is-routed-by-its-thread.md` gives: a daemon
  on somebody's own machine has no public endpoint. Settled by an API for
  app-level tokens, or a configuration token that does not expire.
